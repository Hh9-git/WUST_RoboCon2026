...
int main(void)
{
  ...
  MX_UART8_Init();
  /* USER CODE BEGIN 2 */
  // HAL_UART_Receive_IT(&huart7, ucRxBuffer, sizeof(ucRxBuffer));
  ...
  while (1)
  {
    // Dist = LDS30B_Analysis(Data);

    // HAL_UART_Transmit_IT(&huart7, ucRxBuffer, sizeof(ucRxBuffer));
    // UART_Print("LDS30B:%.3f\n", Distance);
    Elephant_Run();

    // DJ_MotorRun();
    // UART_Print("%d  %d\n", lds1.distance, lds2.distance);
    if (chassis.lds_y != 0 && lds2.distance != 0)
    {
      LED();
    }
    HAL_Delay(2);
