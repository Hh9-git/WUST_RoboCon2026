...
void Remote_Callback()
{
    if (Remote_control_FS.SWA > 1024)
    {
        ...
        DJ_MotorRun();
        //VRB控制大疆2006电机旋转角度
        float HT_angle = (Remote_control_FS.VRA - 1024) / 671.0f * 360.0f;
        HT_SetPosition(&HT_Motors[7],RAD(HT_angle),0.5,0.2,0);
        HT_Run(&HT_Motors[7]);
        //Left_Y通道控制大疆3508电机上升下降
        float DJ_Y_angle = -(Remote_control_FS.Left_Y - 1024) / 671.0f * 960.0f;
        DJ_SetAngle(&DJ_Motor3508[5], DJ_Y_angle,5000);
        DJ_MotorRun();
        //VRB通道控制x
        // float DJ_X_angle = (Remote_control_FS.VRB - 267) / 1540.0f * 360.0f;
        // DJ_SetAngle(&DJ_Motor3508[4], DJ_X_angle,5000);
        // DJ_MotorRun();
        if (Remote_control_FS.SWC<1024)
        {
            //VRB控制大疆2006电机旋转角度
            float DJ_gripper_angle = (Remote_control_FS.VRB - 1024) / 671.0f * 360.0f;;
            // // float DJ_gripper_angle = (Remote_control_FS.VRB - 403) / 1144.0f * 180.0f;
            // DJ_SetAngle(&DJ_Motor2006[0], 0,10000);
            DJ_SetAngle(&DJ_Motor2006[0], DJ_gripper_angle,20000);
            DJ_MotorRun();
        ...
        else if (Remote_control_FS.SWC==1024)
        {
            ...
            float DJ_gripper_angle = (Remote_control_FS.VRB - 1024) / 671.0f * 360.0f;
            // // float DJ_gripper_angle = (Remote_control_FS.VRB - 403) / 1144.0f * 180.0f;
            // DJ_SetAngle(&DJ_Motor2006[0], 90,10000);
            DJ_SetAngle(&DJ_Motor2006[0], DJ_gripper_angle,20000);
            DJ_MotorRun();
        ...
        {
            //SWB默认向上继电器（高电平触发）关闭，一位一通电磁阀断开，气动夹爪张开
        ...
        {
            // //VRB控制大疆2006电机旋转角度
            // int16_t Suction_cup_angle = (Remote_control_FS.VRB - 2z'z'z'z'z'z'z'z'z'z'z'zzzzzzzzzzzzz67) / 1540.0f * 180.0f;
        ...
        {
            // //VRB控制大疆2006电机旋转角度
            // int16_t Suction_cup_angle = (Remote_control_FS.VRB - 267) / 1540.0f * 180.0f;
